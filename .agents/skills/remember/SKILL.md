...
`memory.md` is a handoff note, not a replacement for the project plan. Before saving memory, reconcile the canonical docs:

- If work shipped or a milestone completed, update `context/progress-tracker.md`.
- If priorities, blockers, or remaining gaps changed, update `context/build-plan.md`.
- If the session produced new reusable UI patterns, run `/imprint` so `context/ui-registry.md` is current.

When a session includes explicit plan/progress updates, treat the matching `context/` documentation as part of the same work item. Update the tracker/build-plan in the same pass as the implementation or status change, so the next agent does not inherit a plan that already drifted from reality.

Do not save a memory that says something is done while `context/build-plan.md` still lists it as open, or that says something is next while `context/progress-tracker.md` already records it as shipped. If the docs intentionally disagree because work is on an unmerged branch or blocked externally, state that explicitly in both the memory and the relevant context file.
...
- `context/build-plan.md`, `context/progress-tracker.md` — canonical plan/progress state
